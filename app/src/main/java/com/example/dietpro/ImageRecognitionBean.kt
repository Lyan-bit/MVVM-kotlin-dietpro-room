...
class ImageRecognitionBean(c: Context) {
    private var model: ImageRecognitionViewModel = ImageRecognitionViewModel.getInstance(c)
    private var modelMeal: MealCRUDViewModel = MealCRUDViewModel.getInstance(c)
    ...
    suspend fun isImageRecognitionError(): Boolean {
	    errors.clear()
        instanceMeal = modelMeal.getMealByPK2(meal)
    ...
    suspend fun imageRecognition (): String {
        return model.imageRecognition(dimages!!)
    }
}
